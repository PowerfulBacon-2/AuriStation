...
/obj/machinery/suit_storage_unit/proc/cook()
	...
	if(uv_cycles)
		...
		if(uv_super || (obj_flags & EMAGGED))
			...
			if(helmet)
				helmet.deal_damage(burn_damage * 10, 0, BURN, DAMAGE_FIRE)
			if(suit)
				suit.deal_damage(burn_damage * 10, BURN, DAMAGE_FIRE)
			if(mod)
				mod.deal_damage(burn_damage * 10, BURN, DAMAGE_FIRE)
			if(mask)
				mask.deal_damage(burn_damage * 10, 0, BURN, DAMAGE_FIRE)
			if(storage)
				storage.deal_damage(burn_damage * 10, 0, BURN, DAMAGE_FIRE)

...
/obj/projectile/energy/electrode/on_hit(mob/living/target, blocked = 0, pierce_hit)
	...
	target.apply_status_effect(
		...
		fired_from,
		initial_firer_weakref?.resolve() || firer,
		tase_stamina,
		100 WATT,
		"\the [src]\s",
		initial(range) + 2,
...
/datum/status_effect/tased
	...
	/// The zone that we are targeting
	VAR_FINAL/def_zone = null
	/// Looping sound
	VAR_PRIVATE/datum/looping_sound/taser/taser_loop
...
/datum/status_effect/tased/on_creation(
	...
	if (new_owner.has_status_effect(type))
		var/datum/status_effect/tased/original = new_owner.has_status_effect(type)
		original.end_tase()
...
/datum/status_effect/tased/proc/do_tase_with(atom/with_what, seconds_between_ticks)
	...
	if(istype(taser, /obj/machinery))
		...
		if(!taser_machine.is_operational)
			return FALSE
		if(!taser_machine.directly_use_power(energy_drain * seconds_between_ticks))
			return FALSE
		return TRUE
	...
	if(istype(taser, /obj/item/mecha_parts/mecha_equipment))
		...
		if(!taser_equipment.chassis \
			...
			|| !taser_equipment.chassis.use_power(energy_drain * seconds_between_ticks))
			return FALSE
		return TRUE
...
/datum/status_effect/tased/on_apply()
	...
	// Apply the first tase
	tase_target(1)

	taser_loop = new(owner, TRUE)
...
/datum/status_effect/tased/on_remove()
	...
	if(!QDELETED(tase_line))
		...
		tase_line = null
	QDEL_NULL(taser_loop)

/datum/status_effect/tased/tick()
	var/seconds_between_ticks = initial(tick_interval) / (1 SECONDS)
...
/datum/status_effect/tased/proc/try_remove_taser_async(mob/living/remover)
	...
	// Removes quickly, and allows movement but doesn't allow the use of any other items at the same time
	if(!do_after(remover, 1 SECONDS, owner, timed_action_flags = IGNORE_USER_LOC_CHANGE | IGNORE_TARGET_LOC_CHANGE, extra_checks = CALLBACK(src, PROC_REF(try_remove_taser_checks)), interaction_key = id))
		return
	remover.visible_message(

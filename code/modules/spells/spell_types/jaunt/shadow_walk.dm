...
/obj/effect/dummy/phased_mob/shadow/process(delta_time)
	...
	if(light_amount < 0.2 && !QDELETED(jaunter) && isliving(jaunter)) //heal in the dark
		...
		living_jaunter.heal_overall_injuries(BRUTE, healing_rate * delta_time, BODYTYPE_ORGANIC)
		living_jaunter.heal_overall_injuries(BURN, healing_rate * delta_time, BODYTYPE_ORGANIC)

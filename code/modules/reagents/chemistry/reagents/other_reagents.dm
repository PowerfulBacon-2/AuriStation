...
/datum/reagent/blood/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
	if(data && data["viruses"])
		for(var/thing in data["viruses"])
			...
			if((method == TOUCH || method == VAPOR) && (D.spread_flags & DISEASE_SPREAD_CONTACT_FLUIDS))
				exposed_mob.ContactContractDisease(D)
			else //ingest, patch or inject
				exposed_mob.ForceContractDisease(D)
...
/datum/reagent/blood/expose_turf(turf/exposed_turf, reac_volume)//splash the blood all over the place
	...
	var/obj/effect/decal/cleanable/blood/blood = locate() in exposed_turf //find some blood here
	if(!blood)
...
/datum/reagent/vaccine/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
	if(islist(data) && (method == INGEST || method == INJECT))
		for(var/datum/disease/disease in exposed_mob.diseases)
			if(disease.GetDiseaseID() in data)
				disease.cure()
...
/datum/reagent/water/expose_turf(turf/exposed_turf, reac_volume)
	...
	if(hotspot && !isspaceturf(open_turf))
		if(open_turf.air)
			var/datum/gas_mixture/mix = open_turf.air
			mix.temperature = (max(min(mix.return_temperature() - cooling_temperature * 1000, mix.return_temperature() / cooling_temperature), TCMB))
			mix.react(src)
			qdel(hotspot)

	var/obj/effect/acid/acid = (locate(/obj/effect/acid) in open_turf)
	acid?.acid_level = max(acid.acid_level - reac_volume * 50, 0)
...
/datum/reagent/fuel/unholywater/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	if(IS_CULTIST(affected_mob))
		...
		affected_mob.adjustExhaustion(-10 * REM * delta_time, 0)
...
/datum/reagent/lube/expose_turf(turf/exposed_turf, reac_volume)
	. = ..()
	if(!isopenturf(exposed_turf))
		return
	...
	var/turf/open/open_turf = exposed_turf
	if(reac_volume >= 1)
		open_turf.MakeSlippery(lube_kind, 15 SECONDS, min(reac_volume * 2 SECONDS, 120))
...
/datum/reagent/spraytan/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume, show_message = 1)
	. = ..()
	...
	if(method == PATCH || method == VAPOR)
		...
		if(human.dna.species.id == SPECIES_HUMAN)
			switch(human.skin_tone)
				...
				if("indian")
					human.skin_tone = "african1"
				if("arab")
					human.skin_tone = "indian"
				if("asian2")
					human.skin_tone = "arab"
				if("asian1")
					human.skin_tone = "asian2"
				if("mediterranean")
					human.skin_tone = "african1"
				if("latino")
					human.skin_tone = "mediterranean"
				if("caucasian3")
					human.skin_tone = "mediterranean"
				if("caucasian2")
					human.skin_tone = pick("caucasian3", "latino")
				if("caucasian1")
		...
		if(MUTCOLORS in human.dna.species.species_traits) //take current alien color and darken it slightly
			var/newcolor = ""
			var/string = human.dna.features["mcolor"]
			...
			if(ReadHSV(newcolor)[3] >= ReadHSV("#7F7F7F")[3])
				human.dna.features["mcolor"] = newcolor
...
/datum/reagent/spraytan/overdose_start(mob/living/carbon/affected_mob)
	...
	if(ishuman(affected_mob))
		...
		affected_human.facial_hair_color = "000"
		affected_human.hair_color = "000"
		...
		if(affected_human.dna.species.use_skintones)
			...
			affected_human.dna.features["mcolor"] = "f80"
	...
	if(ishuman(affected_mob))
		...
		if(DT_PROB(3.5, delta_time))
			if(affected_human.w_uniform)
				affected_human.visible_message(pick("<b>[affected_human]</b>'s collar pops up without warning.", "<b>[affected_human]</b> flexes [affected_human.p_their()] arms."))
			else
				affected_human.visible_message("<b>[affected_human]</b> flexes [affected_human.p_their()] arms.")
		if(DT_PROB(1, delta_time))
			affected_human.say(pick("Shit was SO cash.", "Duuuuuude. Yeah bro.", "Check my muscles, broooo!", "Hell yeah brooo!"), forced = TRUE)
...
/datum/reagent/mutationtoxin/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	if(!ishuman(affected_mob))
		return
	...
	if(!(affected_human.dna?.species) || !(affected_human.mob_biotypes & MOB_ORGANIC))
		return
...
/datum/reagent/aslimetoxin/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
	if(method != TOUCH)
...
/datum/reagent/gluttonytoxin/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
...
/datum/reagent/serotrotium
	...
	chemical_flags = CHEMICAL_NOT_SYNTH | CHEMICAL_RNG_FUN | CHEMICAL_RNG_BOTANY
	metabolization_rate = 0.25 * REAGENTS_METABOLISM
	taste_description = "bitterness"

/datum/reagent/serotrotium/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	if(ishuman(affected_mob) && DT_PROB(3.5, delta_time))
...
/datum/reagent/copper/expose_obj(obj/exposed_obj, reac_volume)
	...
	if(istype(exposed_obj, /obj/item/stack/sheet/iron))
		var/obj/item/stack/sheet/iron/iron = exposed_obj
		reac_volume = min(reac_volume, iron.amount)
		new /obj/item/stack/sheet/bronze(get_turf(iron), reac_volume)
		iron.use(reac_volume)
...
/datum/reagent/mercury/lead_acetate
	...
	color = "#AAAAAA"
	chemical_flags = CHEMICAL_RNG_GENERAL | CHEMICAL_RNG_FUN // because brain damage is fun
	taste_description = "sweetness"
	taste_mult = 3
...
/datum/reagent/carbon/expose_turf(turf/exposed_turf, reac_volume)
	...
	var/obj/effect/decal/cleanable/dirt/dirt = locate() in exposed_turf.contents
	if(!dirt)
		new /obj/effect/decal/cleanable/dirt(exposed_turf)
...
/datum/reagent/lithium
	...
	reagent_state = SOLID
	color = "#808080" // rgb: 128, 128, 128
	chemical_flags = CHEMICAL_BASIC_ELEMENT | CHEMICAL_RNG_GENERAL | CHEMICAL_RNG_FUN // because it is fun
	taste_description = "metal"
...
/datum/reagent/space_cleaner/sterilizine/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
	. = ..()
	if(!iscarbon(exposed_mob))
		return
...
/datum/reagent/uranium/expose_turf(turf/exposed_turf, reac_volume)
	...
	var/obj/effect/decal/cleanable/greenglow/goop = locate() in exposed_turf.contents
	if(!goop)
		goop = new/obj/effect/decal/cleanable/greenglow(exposed_turf)
	if(!QDELETED(goop))
		goop.reagents.add_reagent(type, reac_volume)
...
/datum/reagent/bluespace/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
	. = ..()
	if(method == TOUCH || method == VAPOR)
...
/datum/reagent/fuel/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)//Splashing people with welding fuel to make them easy to ignite!
	if(method == TOUCH || method == VAPOR)
...
/datum/reagent/space_cleaner/ez_clean/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
	. = ..()
	if((method == TOUCH || method == VAPOR) && !issilicon(exposed_mob))
		exposed_mob.adjustBruteLoss(1.5, updating_health = FALSE)
		exposed_mob.adjustFireLoss(1.5, updating_health = TRUE)
...
/datum/reagent/nanomachines/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume, show_message = 1, touch_protection = 0)
	if((method in list(PATCH, INGEST, INJECT)) || (method == VAPOR && prob(min(reac_volume, 100) * (1 - touch_protection))))
...
/datum/reagent/xenomicrobes/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume, show_message = 1, touch_protection = 0)
	if((method in list(PATCH, INGEST, INJECT)) || (method == VAPOR && prob(min(reac_volume, 100) * (1 - touch_protection))))
...
/datum/reagent/fungalspores/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume, show_message = 1, touch_protection = 0)
	if((method in list(PATCH, INGEST, INJECT)) || (method == VAPOR && prob(min(reac_volume, 100) * (1 - touch_protection))))
...
/datum/reagent/snail/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume, show_message = 1, touch_protection = 0)
	if((method in list(PATCH, INGEST, INJECT)) || (method == VAPOR && prob(min(reac_volume, 100) * (1 - touch_protection))))
...
/datum/reagent/carpet
	...
	chemical_flags = NONE //Way too many of these to be fair to the bartender!
...
/datum/reagent/carpet/royal/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	switch(affected_mob.mind?.assigned_role)
		if(JOB_NAME_CHIEFMEDICALOFFICER, JOB_NAME_CAPTAIN, JOB_NAME_CHIEFENGINEER, JOB_NAME_RESEARCHDIRECTOR, JOB_NAME_HEADOFPERSONNEL)
			if(DT_PROB(5, delta_time))
...
/datum/reagent/drying_agent/expose_turf(turf/exposed_turf, reac_volume)
	. = ..()
	if(!isopenturf(exposed_turf))
		return

	var/turf/open/open_turf = exposed_turf
	open_turf.MakeDry(ALL, TRUE, reac_volume * 5 SECONDS)

/datum/reagent/drying_agent/expose_obj(obj/exposed_obj, reac_volume)
	. = ..()
	if(exposed_obj.type == /obj/item/clothing/shoes/galoshes)
		new /obj/item/clothing/shoes/galoshes/dry(get_turf(exposed_obj))
		qdel(exposed_obj)
...
/datum/reagent/romerol
	...
	chemical_flags = CHEMICAL_NOT_SYNTH | CHEMICAL_RNG_FUN // romerol zombie outbreak from a maint pill? how can this go not fun.
	metabolization_rate = INFINITY
	taste_description = "brains"

/datum/reagent/romerol/expose_mob(mob/living/carbon/exposed_mob, method = TOUCH, reac_volume)
	. = ..()
	if(!exposed_mob.get_organ_slot(ORGAN_SLOT_ZOMBIE))
		var/obj/item/organ/zombie_infection/infection = new()
		infection.Insert(exposed_mob)
...
/datum/reagent/magillitis/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	if((ismonkey(affected_mob) || ishuman(affected_mob)) && current_cycle >= 10)
...
/datum/reagent/peaceborg/inabizine/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	...
	if(DT_PROB(10, delta_time))
		to_chat(affected_mob, "Your muscles relax...")
...
/datum/reagent/peaceborg/tire/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	if(affected_mob.getExhaustion() < (45 - healthcomp))	//At 50 health you would have 200 - 150 health meaning 50 compensation. 60 - 50 = 10, so would only do 10-19 stamina.)
		affected_mob.adjustExhaustion(10 * REM * delta_time)
...
/datum/reagent/tranquility
	...
	chemical_flags = CHEMICAL_NOT_SYNTH | CHEMICAL_RNG_GENERAL | CHEMICAL_RNG_FUN
	taste_description = "inner peace"

/datum/reagent/tranquility/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume, show_message = 1, touch_protection = 0)
	if((method in list(PATCH, INGEST, INJECT)) || (method == VAPOR && prob(min(reac_volume, 100) * (1 - touch_protection))))
...
/datum/reagent/eldritch/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	if(IS_HERETIC(affected_mob))
		...
		affected_mob.adjustExhaustion(-10 * REM * delta_time, updating_health = FALSE)
...
/datum/reagent/helgrasp/on_mob_add(mob/living/carbon/affected_mob, amount)
	. = ..()
	...
	playsound(affected_mob, 'sound/misc/ahaha.ogg', 80, TRUE, -1) //Very obvious tell so people can be ready
...
/datum/reagent/medicine/eldritchkiss/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()

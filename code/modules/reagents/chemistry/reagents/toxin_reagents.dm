...
/datum/reagent/toxin/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	if(toxpwr)
		affected_mob.adjustToxLoss(toxpwr * REM * delta_time, updating_health = FALSE)
...
/datum/reagent/toxin/mutagen/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
	. = ..()
	if(!exposed_mob.can_mutate() || !iscarbon(exposed_mob))
		return // No robots, AIs, aliens, Ians or other mobs should be affected by this.

	var/mob/living/carbon/exposed_carbon = exposed_mob

	if((method == VAPOR && prob(min(33, reac_volume))) || (method in list(INGEST, PATCH, INJECT)))
		exposed_carbon.random_mutate_unique_identity()
		if(prob(positive_mutation_prob))
			exposed_carbon.easy_random_mutate(POSITIVE)
		else
			exposed_carbon.easy_random_mutate(NEGATIVE+MINOR_NEGATIVE)

		exposed_carbon.updateappearance()
		exposed_carbon.domutcheck()

/datum/reagent/toxin/mutagen/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	affected_mob.adjustToxLoss(0.5 * delta_time * REM, updating_health = TRUE)
...
/datum/reagent/toxin/carpotoxin/on_mob_metabolize(mob/living/carbon/affected_mob)
	. = ..()
...
/datum/reagent/toxin/zombiepowder/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	if(current_cycle >= 10) // delayed activation for toxin
		affected_mob.adjustExhaustion((current_cycle - 5) * REM * delta_time, updating_health = FALSE)
		. = UPDATE_MOB_HEALTH
	if(affected_mob.getExhaustion() >= 145 && !HAS_TRAIT(affected_mob, TRAIT_FAKEDEATH)) // fake death tied to stamina for interesting interactions - 23 ticks to fake death with pure ZP
...
/datum/reagent/toxin/plantbgone/expose_obj(obj/exposed_obj, reac_volume)
	...
	if(istype(exposed_obj, /obj/structure/alien/weeds))
		...
		vine.on_chem_effect(src)

/datum/reagent/toxin/plantbgone/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
	if(method == VAPOR && iscarbon(exposed_mob))
		var/mob/living/carbon/exposed_carbon = exposed_mob
		if(!exposed_carbon.wear_mask) // If not wearing a mask
...
/datum/reagent/toxin/chloralhydrate/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	switch(current_cycle)
		...
		if(51 to INFINITY)
			affected_mob.Sleeping(40 * REM * delta_time)
			affected_mob.adjustToxLoss(1 * (current_cycle - 50) * REM * delta_time, updating_health = FALSE)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/toxin/fakebeer/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	switch(current_cycle)
		...
		if(51 to INFINITY)
			affected_mob.Sleeping(40 * REM * delta_time)
			affected_mob.adjustToxLoss((current_cycle - 50) * REM * delta_time, updating_health = FALSE)
			return UPDATE_MOB_HEALTH
...
/datum/reagent/toxin/staminatoxin/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	data = max(data - 1, 3)
	affected_mob.adjustExhaustion(data * REM * delta_time, updating_health = FALSE)
	return UPDATE_MOB_HEALTH
...
/datum/reagent/toxin/venom/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	toxpwr = 0.1 * volume
	affected_mob.adjustBruteLoss(0.3 * volume * REM * delta_time, updating_health = FALSE)
...
/datum/reagent/toxin/spidervenom/on_mob_metabolize(mob/living/carbon/affected_mob)
	. = ..()
	if(SEND_SIGNAL(affected_mob, COMSIG_HAS_NANITES))
		for(var/datum/component/nanites/nanites in affected_mob.datum_components)
			for(var/datum/nanite_program/program in nanites.programs)
				program.software_error(1) //all programs are destroyed, nullifying all nanites

/datum/reagent/toxin/spidervenom/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	...
	if(affected_mob.getExhaustion() <= 70) //Will never stamcrit
		affected_mob.adjustExhaustion(min(volume * 1.5, 15) * REM * delta_time, updating_health = FALSE)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/toxin/cyanide/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	if(DT_PROB(4, delta_time))
		to_chat(affected_mob, span_warning("You feel horrendously weak!"))
...
/datum/reagent/toxin/itching_powder/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	if(DT_PROB(8, delta_time))
		...
		affected_mob.adjustBruteLoss(0.2*REM, updating_health = FALSE)
		. = UPDATE_MOB_HEALTH
	if(DT_PROB(8, delta_time))
	...
	if(DT_PROB(13, delta_time))
		switch(rand(1, 3))
			...
			if(2)
				...
				affected_mob.adjustOxyLoss(rand(5,25), updating_health = FALSE)
				return UPDATE_MOB_HEALTH
			if(3)
...
/datum/reagent/toxin/amanitin/on_mob_delete(mob/living/carbon/affected_mob)
	. = ..()
	var/toxdamage = current_cycle * 3 * REM
	affected_mob.log_message("has taken [toxdamage] toxin damage from amanitin toxin", LOG_ATTACK)
...
/datum/reagent/toxin/lipolicide/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	affected_mob.adjust_nutrition(-3 * REM * delta_time) // making the chef more valuable, one meme trap at a time
...
/datum/reagent/toxin/rotatium/on_mob_end_metabolize(mob/living/carbon/affected_mob)
	. = ..()
	if(affected_mob?.hud_used)
		var/atom/movable/plane_master_controller/pm_controller = affected_mob.hud_used.plane_master_controllers[PLANE_MASTERS_GAME]
		for(var/key in pm_controller.controlled_planes)
...
/datum/reagent/toxin/acid/expose_mob(mob/living/exposed_mob, method = TOUCH, reac_volume)
...
/datum/reagent/toxin/acid/fluacid/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	affected_mob.adjustFireLoss(current_cycle / 15 * REM * delta_time, updating_health = FALSE)
	return UPDATE_MOB_HEALTH
...
/datum/reagent/toxin/delayed/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	...
	if(current_cycle > delay)
		...
		affected_mob.adjustToxLoss(actual_toxpwr * REM * delta_time, 0)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/toxin/bonehurtingjuice/on_mob_metabolize(mob/living/carbon/affected_mob)
	. = ..()
...
/datum/reagent/toxin/bonehurtingjuice/on_mob_life(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	affected_mob.adjustExhaustion(7.5 * REM * delta_time, updating_health = FALSE)
	...
	if(DT_PROB(10, delta_time))
		switch(rand(1, 3))
			...
			if(2)
				affected_mob.say("*custom " + pick("oofs softly.", "looks like their bones hurt.", "grimaces, as though their bones hurt."), forced = /datum/reagent/toxin/bonehurtingjuice)
			if(3)
...
/datum/reagent/toxin/bonehurtingjuice/overdose_process(mob/living/carbon/affected_mob, delta_time, times_fired)
	. = ..()
	if(DT_PROB(2, delta_time)) //big oof
		var/selected_part

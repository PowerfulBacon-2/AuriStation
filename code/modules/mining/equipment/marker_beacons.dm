...
/obj/item/stack/marker_beacon/attack_self(mob/user)
	...
	if(use(1))
		...
		var/obj/structure/marker_beacon/beacon = new(user.loc, picked_color)
...
/obj/structure/marker_beacon/attack_hand(mob/user, list/modifiers)
	. = ..()
	if(.)
		return
	...
	if(do_after(user, remove_speed, target = src))
		...
		beacon.update_appearance()
		transfer_fingerprints_to(beacon)
		if(user.put_in_hands(beacon, TRUE)) //delete the beacon if it fails
...
/obj/structure/marker_beacon/attackby(obj/item/attacking_item, mob/user, params)
	if(istype(attacking_item, /obj/item/stack/marker_beacon))
		var/obj/item/stack/marker_beacon/beacon = attacking_item
		to_chat(user, span_notice("You start picking [src] up..."))
		if(do_after(user, remove_speed, target = src) && beacon.amount + 1 <= beacon.max_amount)
			beacon.add(1)
			playsound(src, 'sound/items/deconstruct.ogg', 50, TRUE)
			qdel(src)
			return

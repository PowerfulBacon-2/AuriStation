...
//what to toggle when toggled with weldingvisortoggle()
	...
	var/list/static/high_pressure_multiplier_types = list(ARMOUR_BLUNT, ARMOUR_PENETRATION)
...
/obj/item/clothing/attack(mob/living/target, mob/living/user, params)
	...
	if((clothing_flags & NOTCONSUMABLE) && (resistance_flags & INDESTRUCTIBLE) && (get_armor_rating(ARMOUR_PENETRATION) != 0))
...
/obj/item/clothing/proc/disable_zone(def_zone, damage_type)
	...
	if(body_parts_covered == NONE) // if there are no more parts to break then the whole thing is kaput
		atom_destruction((damage_type == BRUTE ? DAMAGE_STANDARD : DAMAGE_LASER)) // melee/laser is good enough since this only procs from direct attacks anyway and not from fire/bombs
		return
...
/obj/item/clothing/examine_tags(mob/user)
	...
	else if (get_armor_for_examination().get_rating(ARMOUR_PENETRATION) >= 20)

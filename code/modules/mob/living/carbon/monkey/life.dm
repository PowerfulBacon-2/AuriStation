	...
	if(bodytemperature > BODYTEMP_HEAT_DAMAGE_LIMIT && !HAS_TRAIT(src, TRAIT_RESISTHEAT))
		...
		switch(bodytemperature)
			if(360 to 400)
				...
				take_direct_damage(HEAT_DAMAGE_LEVEL_1, BURN, DAMAGE_FIRE)
			if(400 to 460)
				throw_alert(ALERT_TEMPERATURE, /atom/movable/screen/alert/hot, 2)
				take_direct_damage(HEAT_DAMAGE_LEVEL_2, BURN, DAMAGE_FIRE)
			if(460 to INFINITY)
				throw_alert(ALERT_TEMPERATURE, /atom/movable/screen/alert/hot, 3)
				if(on_fire)
					take_direct_damage(HEAT_DAMAGE_LEVEL_3, BURN, DAMAGE_FIRE)
				else
					take_direct_damage(HEAT_DAMAGE_LEVEL_2, BURN, DAMAGE_FIRE)
		...
		if(!istype(loc, /obj/machinery/cryo_cell))
			...
			switch(bodytemperature)
				if(200 to BODYTEMP_COLD_DAMAGE_LIMIT)
					...
					take_direct_damage(COLD_DAMAGE_LEVEL_1, BURN, DAMAGE_EXISTENTIAL)
				if(120 to 200)
					throw_alert(ALERT_TEMPERATURE, /atom/movable/screen/alert/cold, 2)
					take_direct_damage(COLD_DAMAGE_LEVEL_2, BURN, DAMAGE_EXISTENTIAL)
				if(-INFINITY to 120)
					throw_alert(ALERT_TEMPERATURE, /atom/movable/screen/alert/cold, 3)
					take_direct_damage(COLD_DAMAGE_LEVEL_3, BURN, DAMAGE_EXISTENTIAL)
		else

...
/datum/species/skeleton/handle_chemicals(datum/reagent/chem, mob/living/carbon/human/H, delta_time, times_fired)
	if(chem.type == /datum/reagent/consumable/milk)
		...
		H.heal_overall_injuries(BRUTE, 1)
		H.heal_overall_injuries(BURN, 1)
		H.reagents.remove_reagent(chem.type, chem.metabolization_rate)
		return TRUE
	if(chem.type == /datum/reagent/toxin/bonehurtingjuice)
		H.adjustExhaustion(7.5 * REAGENTS_EFFECT_MULTIPLIER * delta_time, 0)
		...
		if(chem.overdosed)
			if(DT_PROB(2, delta_time) && iscarbon(H)) //big oof
				...
				if(bp)
					...
					H.take_direct_damage(200, BRUTE, DAMAGE_EXISTENTIAL, bp.body_zone)
				else

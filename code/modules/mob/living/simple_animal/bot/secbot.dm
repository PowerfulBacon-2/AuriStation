...
/mob/living/simple_animal/bot/secbot/handle_automated_action()
	...
	switch(mode)
		...
		if(BOT_PREP_ARREST)		// preparing to arrest target
			...
			if( !Adjacent(target) || !isturf(target.loc) ||  target.getExhaustion() < 100)
				back_to_hunt()
				return
		...
		if(BOT_ARREST)
			...
			if(!Adjacent(target) || !isturf(target.loc) || (target.loc != target_lastloc && target.getExhaustion() < 100)) //if he's changed loc and about to get up or not adjacent or got into a closet, we prep arrest again.
				back_to_hunt()
				return

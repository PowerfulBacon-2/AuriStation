...
/datum/disease/revblight/stage_act(delta_time, times_fired)
	...
	affected_mob.adjustExhaustion(1) //Provides gradual exhaustion, but mostly to prevent regeneration and set an upper limit on disease duration to about five minutes
	if(affected_mob.body_position == LYING_DOWN)
		...
		startresting = null
	if(DT_PROB(1.5 * stage, delta_time) && !finalstage && affected_mob.exhaustion <= stage * 25) //no more lesser flavor messages and sparkles after stage 5
		...
		affected_mob.adjustExhaustion(7.5 * delta_time, FALSE) //Where the real exhaustion builds up.
	...
	switch(stage)
		...
		if(5)
			if(affected_mob.exhaustion <= 200) //check required to prevent randomly screaming from limbs being crippled at extreme stamina
				affected_mob.adjustExhaustion(7.5 * delta_time, FALSE) //No longer realistically possible to counteract with stimulants
			if(!finalstage)
				finalstage = TRUE

...
/obj/item/bodypart/proc/drop_limb(special, dismembered)
	...
	if(!special)
		...
		for(var/organ_slot in organ_slots) //internal organs inside the dismembered limb are dropped.
			var/obj/item/organ/organ = phantom_owner.get_organ_slot(organ_slot)
			if (!organ)
				continue

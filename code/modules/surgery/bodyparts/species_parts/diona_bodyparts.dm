///Dionae Body parts, used to be podpeople
/obj/item/bodypart/head/diona
	icon_static = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon_state = "diona_head"
	limb_id = SPECIES_DIONA
	is_dimorphic = FALSE
	uses_mutcolor = TRUE
	bodypart_flags = BODYPART_UNREMOVABLE | BODYPART_PSEUDOPART

/obj/item/bodypart/head/diona/drop_organs(mob/user, violent_removal)
	. = ..()
	new /mob/living/simple_animal/hostile/retaliate/nymph(user.loc)
	qdel(src)

/obj/item/bodypart/chest/diona
	icon_static = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon_state = "diona_chest"
	limb_id = SPECIES_DIONA
	is_dimorphic = FALSE
	uses_mutcolor = TRUE
	bodypart_flags = BODYPART_PSEUDOPART

/obj/item/bodypart/chest/diona/drop_organs(mob/user, violent_removal)
	. = ..()
	new /mob/living/simple_animal/hostile/retaliate/nymph(user.loc)
	qdel(src)

/obj/item/bodypart/arm/left/diona
	icon_static = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon_state = "diona_l_arm"
	limb_id = SPECIES_DIONA
	uses_mutcolor = TRUE
	bodypart_flags = BODYPART_PSEUDOPART

/obj/item/bodypart/arm/left/diona/drop_organs(mob/user, violent_removal)
	. = ..()
	new /mob/living/simple_animal/hostile/retaliate/nymph(user.loc)
	qdel(src)

/obj/item/bodypart/arm/right/diona
	icon_static = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon_state = "diona_r_arm"
	limb_id = SPECIES_DIONA
	uses_mutcolor = TRUE
	bodypart_flags = BODYPART_PSEUDOPART

/obj/item/bodypart/arm/right/diona/drop_organs(mob/user, violent_removal)
	. = ..()
	new /mob/living/simple_animal/hostile/retaliate/nymph(user.loc)
	qdel(src)

/obj/item/bodypart/leg/left/diona
	icon_static = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon_state = "diona_l_leg"
	limb_id = SPECIES_DIONA
	uses_mutcolor = TRUE
	bodypart_flags = BODYPART_PSEUDOPART

/obj/item/bodypart/leg/left/diona/drop_organs(mob/user, violent_removal)
	. = ..()
	new /mob/living/simple_animal/hostile/retaliate/nymph(user.loc)
	qdel(src)

/obj/item/bodypart/leg/right/diona
	icon_static = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon = 'icons/mob/human/species/diona/bodyparts.dmi'
	icon_state = "diona_r_leg"
	limb_id = SPECIES_DIONA
	uses_mutcolor = TRUE
	bodypart_flags = BODYPART_PSEUDOPART

/obj/item/bodypart/leg/right/diona/drop_organs(mob/user, violent_removal)
	. = ..()
	new /mob/living/simple_animal/hostile/retaliate/nymph(user.loc)
	qdel(src)

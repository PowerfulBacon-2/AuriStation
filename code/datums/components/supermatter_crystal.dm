...
/datum/component/supermatter_crystal/proc/blob_hit(datum/source, obj/structure/blob/blob)
	...
	if(blob.get_integrity() > 100)
		...
		blob.take_direct_damage(100, BURN, DAMAGE_EXISTENTIAL)
	else
	...
	if(istype(item, /obj/item/clothing/mask/cigarette))
		...
		if(cig.lit || user.combat_mode)
			...
			user.visible_message(
				span_danger("As [user] lights \their [item] on \the [atom_source], silence fills the room..."),
...
/datum/component/supermatter_crystal/proc/consume(atom/source, atom/movable/consumed_object)
	...
	if(isliving(consumed_object))
		...
		// Dusting Poly creates a power surge
		if(istype(atom_source, /obj/machinery/power/supermatter_crystal) && istype(consumed_mob, /mob/living/simple_animal/parrot/Poly))
			var/datum/round_event_control/supermatter_surge/poly/revenge = new()
			revenge.runEvent()
		...
		if(!iseffect(consumed_object))
			...
			atom_source.investigate_log("has consumed [consumed_object] - [suspicion].", INVESTIGATE_ENGINES)

...
/datum/quirk/blooddeficiency/on_process(delta_time)
	var/mob/living/carbon/human/H = quirk_target
	if(HAS_TRAIT(H, TRAIT_NO_BLOOD) || HAS_TRAIT(H, TRAIT_NO_BLOOD)) //can't lose blood if your species doesn't have any
		return
	if(H.blood.volume > (BLOOD_VOLUME_SAFE - 25)) // just barely survivable without treatment
		H.blood.volume -= 0.275 * delta_time

...
/mob/living/carbon/ContactContractDisease(datum/disease/disease, target_zone)
	...
	if(ishuman(src))
		...
		switch(target_zone)
			if(BODY_ZONE_HEAD)
				if(isobj(infecting_human.head) && infecting_human.head.clothing_flags & THICKMATERIAL)
					return
				if(isobj(infecting_human.wear_mask) && infecting_human.wear_mask.clothing_flags & THICKMATERIAL)
					return
				if(isobj(infecting_human.wear_neck) && infecting_human.wear_neck.clothing_flags & THICKMATERIAL)
					return
			if(BODY_ZONE_CHEST)
				if(isobj(infecting_human.wear_suit) && infecting_human.wear_suit.clothing_flags & THICKMATERIAL)
					return
				if(isobj(infecting_human.w_uniform) && infecting_human.w_uniform.clothing_flags & THICKMATERIAL)
					return
			if(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM)
				if(isobj(infecting_human.wear_suit) && (infecting_human.wear_suit.body_parts_covered & HANDS) && (infecting_human.wear_suit & THICKMATERIAL))
					return
				if(isobj(infecting_human.gloves) && infecting_human.gloves.clothing_flags & THICKMATERIAL)
					return
			if(BODY_ZONE_L_LEG, BODY_ZONE_R_LEG)
				if(isobj(infecting_human.wear_suit) && (infecting_human.wear_suit.body_parts_covered & FEET) && (infecting_human.wear_suit.clothing_flags & THICKMATERIAL))
					return
				if(isobj(infecting_human.shoes) && infecting_human.shoes.clothing_flags & THICKMATERIAL)
					return
...
/mob/living/carbon/CanSpreadAirborneDisease()
	return !((head && (head.flags_cover & HEADCOVERSMOUTH) && (head.clothing_flags & THICKMATERIAL)) || (wear_mask && (wear_mask.flags_cover & MASKCOVERSMOUTH) && (wear_mask.clothing_flags & THICKMATERIAL)))
